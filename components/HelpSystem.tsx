...
const SubnetBitsVisual: React.FC<{ bits: string }> = ({ bits }) => {
  ...
  if (currentVal === 0) {
     return (
        <div className="flex flex-col items-center gap-4 my-4 bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-800">
           <div className="text-sm text-slate-500 dark:text-slate-400 mb-2">{t('help.first_subnet')}</div>
           <div className="flex gap-2">
              {bits.split('').map((b, i) => (
                  <div key={i} className="w-10 h-10 flex items-center justify-center bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 rounded font-mono text-xl font-bold shadow-sm text-slate-800 dark:text-slate-200">
                      {b}
                  </div>
              ))}
           </div>
           <p className="text-xs text-slate-500 dark:text-slate-400">{t('help.start_counting')}</p>
        </div>
     );
  }

  return (
    <div className="flex flex-col items-center gap-2 my-4 bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-800">
      <div className="flex items-center gap-2 opacity-50">
        <span className="text-xs font-mono uppercase w-16 text-right text-slate-500 dark:text-slate-400">{t('help.previous')}</span>
        <div className="flex gap-1">
            {prevBits.split('').map((b, i) => (
                <div key={i} className="w-8 h-8 flex items-center justify-center bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-slate-500 dark:text-slate-400 font-mono font-bold">
                    {b}
                </div>
            ))}
        </div>
      ...
      <div className="flex items-center gap-2">
         ...
         <div className="flex gap-1 justify-center w-full relative">
             <div className="absolute -top-3 text-slate-400 dark:text-slate-500 font-bold text-lg">+</div>
             <span className="text-sm font-bold text-indigo-500 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-full border border-indigo-100 dark:border-indigo-800 mt-1">1</span>
         </div>
      </div>

      <div className="flex items-center gap-2">
        <span className="text-xs font-mono uppercase w-16 text-right font-bold text-slate-700 dark:text-slate-300">{t('help.current')}</span>
        <div className="flex gap-1">
            {bits.split('').map((b, i) => {
                ...
                return (
                    <div key={i} className={`
                        ...
                        ${changed ? 'bg-yellow-50 dark:bg-yellow-900/30 border-yellow-400 dark:border-yellow-600 text-yellow-700 dark:text-yellow-200' : 'bg-white dark:bg-slate-800 border-slate-400 dark:border-slate-600 text-slate-800 dark:text-slate-200'}
                    `}>
                        {b}
                    </div>
                );
      ...
      <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 text-center max-w-xs">
          ...
          <span className="text-yellow-700 dark:text-yellow-400">{t('help.highlight_expl')}</span>
      </p>
    </div>
  );
};
...
const HostRangeVisual: React.FC<{ net: string, first: string, last: string, broadcast: string }> = ({ net, first, last, broadcast }) => {
    ...
    return (
        <div className="space-y-3 my-4">
            <div className="flex items-center gap-3 p-2 rounded bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 opacity-75">
                <div className="w-2 h-full bg-red-400 dark:bg-red-600 rounded-full"></div>
                <div className="flex-1">
                    <div className="text-xs uppercase font-bold text-red-800 dark:text-red-300">Network ID</div>
                    <div className="font-mono text-red-900 dark:text-red-200">{net}</div>
                </div>
                <div className="text-xs text-red-600 dark:text-red-400 italic">{t('help.reserved')} ({t('help.cannot_use')})</div>
            </div>
            
            <div className="flex flex-col gap-1 pl-4 border-l-2 border-emerald-300 dark:border-emerald-700 ml-3">
                <div className="flex items-center gap-2">
                    <div className="text-xs bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 px-2 py-0.5 rounded">{t('help.first_usable')}</div>
                    <div className="font-mono text-slate-700 dark:text-slate-200">{first}</div>
                    <div className="text-xs text-slate-400 dark:text-slate-500">({t('help.network_ip_math')})</div>
                </div>
                <div className="h-4 border-l border-dashed border-slate-300 dark:border-slate-700 ml-2"></div>
                <div className="flex items-center gap-2">
                    <div className="text-xs bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 px-2 py-0.5 rounded">{t('help.last_usable')}</div>
                    <div className="font-mono text-slate-700 dark:text-slate-200">{last}</div>
                    <div className="text-xs text-slate-400 dark:text-slate-500">({t('help.broadcast_ip_math')})</div>
                </div>
            </div>

            <div className="flex items-center gap-3 p-2 rounded bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/30 opacity-75">
                <div className="w-2 h-full bg-amber-400 dark:bg-amber-600 rounded-full"></div>
                <div className="flex-1">
                    <div className="text-xs uppercase font-bold text-amber-800 dark:text-amber-300">Broadcast ID</div>
                    <div className="font-mono text-amber-900 dark:text-amber-200">{broadcast}</div>
                </div>
                <div className="text-xs text-amber-600 dark:text-amber-400 italic">{t('help.reserved')} ({t('help.talks_to_all')})</div>
            </div>
        </div>
    );
};
...
const MaskLogicVisual: React.FC<{ n: number, s: number, h: number, sActive: boolean }> = ({ n, s, h, sActive }) => {
  ...
  const getColor = (type: string, val: number) => {
     // N is always 1 (Green)
     if (type === 'N') return 'bg-green-500 border-green-600 text-white dark:bg-green-700 dark:border-green-600';
     
     // S depends on state
     if (type === 'S') {
         // If val is 1 (New Mask), Green. If val is 0 (Old Mask), Red.
         return val === 1 
            ? 'bg-green-500 border-green-600 text-white dark:bg-green-700 dark:border-green-600' 
            : 'bg-red-400 border-red-500 text-white dark:bg-red-900/80 dark:border-red-800 dark:text-red-200';
     }

     // H is always 0 (Red)
     return 'bg-red-200 border-red-300 text-red-800 dark:bg-red-900/40 dark:border-red-800 dark:text-red-300';
  };
  ...
  return (
    <div className="my-4">
      {/* Legend */}
      <div className="flex gap-4 text-xs mb-3 justify-center bg-slate-50 dark:bg-slate-900 py-2 rounded border border-slate-100 dark:border-slate-800">
         <div className="flex items-center gap-1">
             <div className="w-3 h-3 bg-green-500 dark:bg-green-700 rounded border border-green-600"></div> 
             <span className="font-medium text-slate-600 dark:text-slate-400">{t('help.mask_legend_masked')}</span>
         </div>
         <div className="flex items-center gap-1">
             <div className="w-3 h-3 bg-red-400 dark:bg-red-900/80 rounded border border-red-500 dark:border-red-800"></div> 
             <span className="font-medium text-slate-600 dark:text-slate-400">{t('help.mask_legend_unmasked')}</span>
         </div>
      </div>
                    ...
                    {Array.from({length: 8}).map((_, bit) => {
                        ...
                        return (
                            ...
                            `}>
                                {val}
                                {/* Hover Tooltip */}
                                <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-slate-800 dark:bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-opacity">
                                    {getTooltip(type)}
                                    <div className="absolute -top-1 left-1/2 -translate-x-1/2 border-4 border-transparent border-b-slate-800 dark:border-b-black"></div>
                                </div>
                            </div>
                        )
                    })}
      ...
      {/* Dynamic explanation */}
      <div 
        className="mt-4 text-center text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 p-2 rounded border border-slate-100 dark:border-slate-800"
          ...
          return {
            ...
            content: (
              <div>
                ...
                <p className="mt-2">Калкулаторът използва формулата <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded">2^N ≥ Изисквани</code>, за да определи колко бита трябва да "заемем" от частта за хостове, за да създадем тези нови мрежови индекси.</p>
              </div>
            )
          };
        case 'VIS_BITS':
            ...
            content: (
                <div>
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-2 text-sm">
                        ...
                        <div className="flex items-center gap-2"><div className="w-4 h-4 bg-red-300 dark:bg-red-900 rounded border border-red-400"></div> <span><strong>Хост битове:</strong> Останалите битове, налични за устройства (компютри, телефони и др.) във всяка подмрежа.</span></div>
                    </div>
                </div>
            )
        };
            ...
            return {
                ...
                content: (
                    <div>
                        ...
                        <p className="mt-2 text-red-600 dark:text-red-400 font-semibold">Той е ЗАПАЗЕН.</p>
            ...
            return {
                ...
                content: (
                    <div>
                        ...
                        <p className="mt-2 text-amber-600 dark:text-amber-400 font-semibold">Той е ЗАПАЗЕН.</p>
          ...
          return {
            ...
            content: (
              <div>
                ...
                <p className="mt-2">The calculator uses the formula <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded">2^N ≥ Required</code> to determine how many bits we need to "borrow" from the host portion to create these new network indexes.</p>
              </div>
            )
          };
        case 'VIS_BITS':
            ...
            content: (
                <div>
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-2 text-sm">
                        ...
                        <div className="flex items-center gap-2"><div className="w-4 h-4 bg-red-300 dark:bg-red-900 rounded border border-red-400"></div> <span><strong>Host Bits:</strong> The remaining bits available for devices (computers, phones, etc.) in each subnet.</span></div>
                    </div>
                </div>
            )
        };
            ...
            return {
                ...
                content: (
                    <div>
                        ...
                        <p className="mt-2 text-red-600 dark:text-red-400 font-semibold">It is RESERVED.</p>
            ...
            return {
                ...
                content: (
                    <div>
                        ...
                        <p className="mt-2 text-amber-600 dark:text-amber-400 font-semibold">It is RESERVED.</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 animate-fade-in">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-slide-up text-slate-700 dark:text-slate-300">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 px-6 py-3 text-right border-t border-slate-100 dark:border-slate-800">
            <button onClick={closeHelp} className="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">{t('help.close')}</button>
        </div>
      </div>
    </div>
  );
